...
kd_tree *insert(kd_tree *T, wine *v, int d, int dem)
{
  ...
  if(v->attr[d] < T->key->attr[d]) T->left = insert(T->left, v, (d+1) % dem, dem);
...
void neighbor(wine A, kd_tree *curr, priority_queue<pri> &Q, int D, int Dem, int K)
{
    ...
    double D_Dist = A.attr[D] - curr->key->attr[D];
    kd_tree *near;
    kd_tree *far;
...
int main(int argc, char **argv)
{
     ...
     for (int i =0; i < D; i++) 
    {
     double mean = 0.0;
      ...
      for (int j = 0; j< N; j++)
      {
        wines[j].attr[i] /= stddev;
      }
    }
    
        
    
    wine *randwine = randomize(wines, N);  //MAKE THIS FIRST TOMOROR
    
    int curr_dem = 0;
    for(int i = 0; i < N; i++)
    {
        tree = insert(tree, &randwine[i], curr_dem, D); 
        if(curr_dem < D) curr_dem++;
        else curr_dem = 0;
    }
