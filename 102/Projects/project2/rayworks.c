...
int main()
{
	...
	sphere2.center.x = -.5;
	sphere2.center.y = .15;
	sphere2.center.z = 4.2;
	sphere2.radius = .6;

	PLANE_T plane;
	plane.normal_plane.x = 0;
	plane.normal_plane.y = 1;
	plane.normal_plane.z = 0;
	plane.d = .9;
	...
	for(y = 0; y < w; y++)
	{
		for(x = 0; x < h; x++)
		{
			...
			ray.direction.x = (-0.5 + (x/500.0));
			ray.direction.y = (0.5 - (y/500.0));
			ray.direction.z = 1;

			//normalise ray direction (use normalize function)	
			ray.direction = Normalize(ray.direction);
			
			//Getting the intersect points on the sphere			
			...
			{	
				...
				printf("%c%c%c", 76, 76, 127);  // prints the colored background
								// rounded up from .3 * 255.
