...
void findTheCookie();
void initializeMap(char map[10][10], int cookieLoc[1][2], int monsterLoc[1][2], int robotLoc[1][2]);
void drawMap(char map[10][10], int robotLoc[1][2]);
int userMove(char map[10][10], char move, int robotLoc[1][2], int cookieLoc[1][2], int monsterLoc[1][2]);
...
void findTheCookie(){
	...
	if (check == 1)
		printf("Congradulations!! You found the Cookie! Now exiting.");
	else if (check == -1)
...
void drawMap(char map[10][10], int robotLoc[1][2]){
	...
	for (row = 0; row < 10; row++){
		...
		{
			if (row == robotLoc[0][0] && col == robotLoc[0][1])
				map[row][col] = 'R';
			else
				map[row][col] = '.';
...
int userMove(char map[10][10], char move, int robotLoc[1][2], int cookieLoc[1][2], int monsterLoc[1][2])
{
	...
	while (go == 0){
		...
		if ((robotLoc[0][0] == monsterLoc[0][0]) && (robotLoc[0][1] == monsterLoc[0][1])){
			go = -1;
			
		}
		else if ((robotLoc[0][0] == cookieLoc[0][0]) && (robotLoc[0][1] == cookieLoc[0][1])){
			go = 1;
